...
export function TrackTable({ initialFilter = 'all' }: { initialFilter?: string }) {
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set()); const [editing, setEditing] = useState<Track | null>(null); const [playing, setPlaying] = useState<Track | null>(null);
  ...
  return <div className="page-stack"><header className="page-header"><div><p className="eyebrow">Catálogo</p><h1>Canciones</h1><p>{loading ? 'Consultando catálogo…' : `${total.toLocaleString('es-ES')} canciones encontradas`}</p></div></header>
    ...
    {playing && <section className="audio-player"><div><strong>{playing.title ?? playing.originalTitle}</strong><span>{playing.artist ?? 'Artista desconocido'} · {playing.album ?? 'Sin álbum'}</span></div><audio controls src={api.trackAudioUrl(playing.id)} preload="metadata"/><button className="icon-button" aria-label="Cerrar reproductor" onClick={() => setPlaying(null)}>×</button></section>}
