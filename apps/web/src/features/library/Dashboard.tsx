...
export function Dashboard({ stats, roots, scan, onRootsChanged, onScanChanged, onNavigate }: { stats: Stats; roots: RootGrant[]; scan: ScanJob | null; onRootsChanged: () => void; onScanChanged: (job: ScanJob) => void; onNavigate: () => void }) {
  ...
  if (!stats.audioFiles && !activeScan) return <div className="dashboard-empty">
    ...
    <section className="onboarding-card" aria-labelledby="start-title"><span className="step-number">01</span><p className="eyebrow">Empezar</p><h2 id="start-title">Elige tu biblioteca</h2><p className="muted">Selecciona tu carpeta en el cuadro de macOS. Resonancia la analizará inmediatamente en modo de solo lectura.</p>
