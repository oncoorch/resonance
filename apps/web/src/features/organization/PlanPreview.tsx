...
export function PlanPreview({ plan, roots, mode, settings, onPlanChanged }: { plan: Plan | null; roots: RootGrant[]; mode: Mode; settings: any; onPlanChanged: (plan: Plan) => void }) {
  ...
  useEffect(() => {
    if (!buildJob || buildJob.id === 'starting' || !['queued', 'running'].includes(buildJob.state)) return;
    const timer = window.setInterval(() => void api.planBuild(buildJob.id).then((next) => { setBuildJob(next); if (next.state === 'completed' && next.plan) { onPlanChanged(next.plan); setBusy(false); } if (next.state === 'failed' || next.state === 'cancelled') setBusy(false); }).catch(() => setBusy(false)), 700);
    return () => window.clearInterval(timer);
  }, [buildJob, onPlanChanged]);
  ...
  return <div className="page-stack"><header className="page-header"><div><p className="eyebrow">Centro de ejecución</p><h1>Ejecutar RESONANCE</h1><p>El flujo completo está aquí: elegir carpetas, revisar opciones, ejecutar revisión, aprobar y aplicar.</p></div>{plan && <span className="revision">Revisión {plan.revision}</span>}</header>
    ...
    {buildJob && <section className="work-monitor" role="status" aria-live="polite"><div className="progress-head"><div><p className="eyebrow">Proceso de revisión</p><h2>{buildJob.phase}</h2></div><strong>{buildJob.total > 0 ? `${Math.round((buildJob.processed / buildJob.total) * 100)}%` : '0%'}</strong></div><div className={`progress-track ${buildJob.total ? '' : 'indeterminate'}`}><i style={buildJob.total ? { width: `${Math.round((buildJob.processed / buildJob.total) * 100)}%` } : undefined}/></div><div className="progress-meta"><span>{buildJob.processed.toLocaleString('es-ES')} procesados</span><span>{buildJob.total.toLocaleString('es-ES')} totales</span><span>{buildJob.conflicts ?? 0} para revisión</span><span>{buildJob.warnings ?? 0} advertencias</span></div>{buildJob.current && <div className="current-file"><b>Ahora:</b><code>{buildJob.current}</code></div>}{buildJob.error && <Notice tone="error">{buildJob.error}</Notice>}<div className="button-row"><button className="button danger" onClick={() => void cancelBuild()} disabled={buildJob.id === 'starting' || !['queued','running'].includes(buildJob.state)}>Cancelar revisión</button></div></section>}
    ...
    {!plan ? <EmptyState icon="duplicates" title="Aún no hay revisión">Cuando ejecutes la revisión verás conteos, conflictos y el botón para aprobar/aplicar.</EmptyState> : <>
      <div className="plan-summary"><article><span>Procesados</span><strong>{plan.items.length.toLocaleString('es-ES')}</strong></article><article><span>Ejecutables ahora</span><strong>{(plan.executable ?? plan.items.filter((item) => item.selected).length).toLocaleString('es-ES')}</strong></article><article><span>Para revisar</span><strong className={plan.conflicts ? 'danger-text' : ''}>{plan.conflicts.toLocaleString('es-ES')}</strong></article><article><span>Duplicación detectada</span><strong>{plan.audit?.audioFiles ? `${Math.round((plan.conflicts / plan.audit.audioFiles) * 1000) / 10}%` : '0%'}</strong></article></div>
      <div className="plan-summary"><article><span>Advertencias</span><strong>{plan.warnings ?? 0}</strong></article><article><span>Espacio a escribir</span><strong>{formatBytes(plan.estimatedBytes)}</strong></article><article><span>Total auditado</span><strong>{(plan.audit?.audioFiles ?? plan.items.length).toLocaleString('es-ES')}</strong></article><article><span>Estado</span><strong>{plan.state}</strong></article></div>
      ...
      {plan.space && <Notice tone={plan.space.sufficientForCopy ? 'success' : 'warning'}>{plan.space.sufficientForCopy ? `Espacio OK para copiar: requiere ${formatBytes(plan.space.requiredBytes)} y hay ${formatBytes(plan.space.availableBytes)} disponibles.` : plan.space.sameVolume ? `Espacio insuficiente para duplicar: requiere ${formatBytes(plan.space.requiredBytes)} y hay ${formatBytes(plan.space.availableBytes)}. Como origen y destino están en el mismo disco, la alternativa segura es un modo avanzado de MOVER: renombrar/mover con verificación y liberar el espacio del origen después de confirmar cada archivo.` : `Espacio insuficiente para copiar: requiere ${formatBytes(plan.space.requiredBytes)} y hay ${formatBytes(plan.space.availableBytes)}. Libera espacio o usa un destino en el mismo disco para poder mover con verificación.`}</Notice>}
