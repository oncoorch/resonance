...
import type { AppSettings, Plan, RootGrant, ScanJob, Stats, ViewId } from '../types';
...
export function App() {
  ...
  const [settings, setSettings] = useState<AppSettings | null>(null); const [connected, setConnected] = useState<boolean | null>(null); const [bootstrapError, setBootstrapError] = useState<string | null>(null);
  ...
  useEffect(() => {
    void (async () => {
      try {
        await api.bootstrap();
        setAuth('ready');
      } catch (error) { setBootstrapError(error instanceof Error ? error.message : 'El servicio local no está disponible'); setAuth('error'); }
    })();
  }, []);
  useEffect(() => { if (auth === 'ready') void loadDashboard(); }, [auth, loadDashboard]);

  if (auth !== 'ready') return <main className="pairing-shell"><section className="pairing-card" aria-busy={auth === 'loading'}><div className="brand-mark"><Icon name="wave"/></div><p className="eyebrow">Resonancia · servicio local</p><h1>{auth === 'loading' ? 'Abriendo tu biblioteca…' : 'No se pudo conectar'}</h1>{auth === 'error' && <><p className="muted">Comprueba que el servicio local siga abierto y vuelve a intentar.</p><button className="button primary" type="button" onClick={() => window.location.reload()}>Reintentar</button>{bootstrapError && <Notice tone="warning">{bootstrapError}</Notice>}</>}</section></main>;

  const title = ({ library: 'Biblioteca', tracks: 'Canciones', artists: 'Artistas', albums: 'Álbumes', genres: 'Géneros', duplicates: 'Duplicados', unidentified: 'Sin identificar', playlists: 'Playlists', history: 'Historial', settings: 'Configuración', organization: 'Organización' } as Record<ViewId, string>)[view];
  let content;
