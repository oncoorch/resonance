...
const defaultSettings: AppSettings = { mode: 'safe', updateTags: false, artwork: false, preserveArtwork: true, preferLossless: true, language: 'es', theme: 'light', provider: { musicbrainzEnabled: false, openaiEnabled: false, webSearchEnabled: false, openaiConfigured: false, model: null, maxRequests: 100, maxWebRequests: 10 } };

export function App() {
  const [view, setView] = useState<ViewId>('library'); const [menuOpen, setMenuOpen] = useState(false);
  const [roots, setRoots] = useState<RootGrant[]>([]); const [stats, setStats] = useState<Stats>(EMPTY_STATS); const [scan, setScanJob] = useState<ScanJob | null>(null); const [plan, setPlan] = useState<Plan | null>(null);
  ...
  const applySettings = useCallback((next: AppSettings) => { setSettings(next); }, []);
  const loadDashboard = useCallback(async () => { try { const [statsResult, rootsResult] = await Promise.all([api.stats(), api.roots()]); setStats(statsResult); setRoots(rootsResult.roots); } catch { /* ignore */ } }, []);
  const loadRoots = useCallback(() => void loadDashboard(), [loadDashboard]);
  const onScanChanged = useCallback((job: ScanJob) => { setScanJob(job); if (job?.state === 'completed' || job?.state === 'failed' || job?.state === 'cancelled') void loadDashboard(); }, []);
  ...
  const resetState = () => { setRoots([]); setStats(EMPTY_STATS); setScanJob(null); setPlan(null); applySettings(defaultSettings); setView('settings'); };
  let content;
  if (view === 'library') content = <Dashboard stats={stats} roots={roots} scan={scan} onRootsChanged={loadRoots} onScanChanged={onScanChanged} onNavigate={() => setView('organization')}/>;
  ...
  else if (view === 'organization') content = <PlanPreview plan={plan} roots={roots} mode={settings.mode} settings={settings} onPlanChanged={setPlan}/>;
  else if (view === 'playlists') content = <Playlists/>;
  else if (view === 'history') content = <History/>;
  else content = <ProviderSettings roots={roots} onRootsChanged={loadRoots} onScanChanged={onScanChanged} onSettingsChanged={applySettings} onReset={resetState}/>;
