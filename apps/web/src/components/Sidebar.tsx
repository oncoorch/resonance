...
const labels = {
  es: { music: 'Tu música', library: 'Biblioteca', tracks: 'Canciones', artists: 'Artistas', albums: 'Álbumes', genres: 'Géneros', review: 'Revisión', organization: 'Ejecutar', duplicates: 'Duplicados', unidentified: 'Sin identificar', collections: 'Colecciones', playlists: 'Playlists', history: 'Historial', settings: 'Configuración', locked: ' · bloqueadas', local: 'Solo en este dispositivo' },
  en: { music: 'Your music', library: 'Library', tracks: 'Songs', artists: 'Artists', albums: 'Albums', genres: 'Genres', review: 'Review', organization: 'Run', duplicates: 'Duplicates', unidentified: 'Unidentified', collections: 'Collections', playlists: 'Playlists', history: 'History', settings: 'Settings', locked: ' · locked', local: 'Only on this device' },
};
...
export function Sidebar({ active, onNavigate, open, onClose, badges, playlistsUnlocked, language }: { active: ViewId; onNavigate: (id: ViewId) => void; open: boolean; onClose: () => void; badges: { duplicates: number; unidentified: number }; playlistsUnlocked: boolean; language: Language }) {
  ...
  const groups: Array<{ label: string; items: Array<{ id: ViewId; label: string; badge?: keyof typeof badges }> }> = [
    ...
    { label: t.review, items: [{ id: 'organization', label: t.organization }, { id: 'duplicates', label: t.duplicates, badge: 'duplicates' }, { id: 'unidentified', label: t.unidentified, badge: 'unidentified' }] },
